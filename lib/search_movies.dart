  ...
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        body: Container(
          ...
          child: FutureBuilder(
            future:
                new restapi.RestAPI().get_movie_by_name(searchController.text),
            builder: (context, snapshot) {
              if (snapshot.hasData) {
                if ((snapshot.data.toString().contains("Erro")) &&
                    (searchController.text.isEmpty)) {
                  return Container(
                    padding: EdgeInsets.all(5.0),
                    child: Center(child: Text("Procure pelo seu filme.")),
                  );
                } else {
                  if ((snapshot.data.toString().contains("ErroCliente")) ||
                      (snapshot.data.toString().contains("ErroServidor"))) {
                    return Container(
                        padding: EdgeInsets.all(5.0),
                        child: Center(
                            child: Text(
                          ...
                          return GestureDetector(
                            onTap: () => {
                              Navigator.push(
                                  context,
                                  MaterialPageRoute(
                                      builder: (context) => Movie_Info(
                                          id_movie: dd['results'][idx]['id'])))
                            },
                            child: SizedBox(
                              child: Card(
                                  child: Padding(
